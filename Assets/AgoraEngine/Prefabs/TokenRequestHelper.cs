...
namespace agora_utilities
{
    ...
    public static class TokenRequestHelper
    {
        ...
        public static IEnumerator FetchToken(
            string endpoint, string channel, uint userId, string role, int expireSecs, Action<TokenObject> callback = null
        )
        {
            ...
            var url = string.Format(RteTokenEndPointFormatter, endpoint, channel, role, userId, expireSecs);
            UnityWebRequest request = UnityWebRequest.Get(url);
            yield return request.SendWebRequest();

            if (request.result != UnityWebRequest.Result.Success)
            {
                Debug.Log("Request to " + url + " ==> " + request.error);
                callback(null);
                yield break;
            }
